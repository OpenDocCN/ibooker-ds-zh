...
或许你对[表格 6-1](#r-arithmetic)中的最后两个操作符不太熟悉：*modulo*返回除法的余数，*floor division*将除法结果向下取整到最接近的整数。
...
对象可以包含不同类型或*模式*的数据，就像您可能有不同类型的鞋盒一样。[表6-3](#data-types-r)列出了一些常见的数据类型。
