...
![dpj2 2001](img/dpj2_2001.png)

###### 图 20-1\. 章节目标元素

正如本章所示，D3 不仅用于构建 SVG 可视化。您可以将数据绑定到任何 DOM 元素，并使用它来更改其属性和属性或其事件处理回调函数。D3 的数据连接和事件处理（通过`on`方法实现）非常适用于本章的可点击列表和选择框等常见用户界面。¹
...
```py
/* WINNERS LIST */
#nobel-list { overflow: scroll; overflow-x: hidden; } ![1](img/1.png)
...
![1](img/#co_visualizing_individual_winners_CO1-1)
...
要创建这些行，我们的中心`onDataChange`在应用初始化时将调用`updateList`方法，并在用户应用数据过滤器并更改获奖者列表时随后调用（参见“基本数据流”）。`updateList`接收到的数据将具有以下结构：
...
示例 20-1 展示了`updateList`方法。首先按年份对接收到的数据进行排序，然后在移除任何现有行后，用于构建表格行。
...
let updateList = function (data) {
  ...
  rows.join( ![1](img/1.png)
    (enter) => {
      // create any new rows required
      return enter.append('tr').on('click', function (event, d) { ![2](img/2.png)
  ...
  cells.join('td').text(d => d) ![5](img/5.png)
  // Display a random winner if data is available
  if (data.length) { ![6](img/6.png)
    displayWinner(data[Math.floor(Math.random() * data.length)])
  }
}
```

![1](img/#co_visualizing_individual_winners_CO2-1)

一个现在熟悉的连接模式，使用绑定的获奖者数据创建和更新列表项。

![2](img/#co_visualizing_individual_winners_CO2-2)

当用户点击一行时，此点击处理函数将将绑定到该行的获奖者数据传递给`displayWinner`方法，后者将相应地更新生物框。

![3](img/#co_visualizing_individual_winners_CO2-3)

这个自定义的`exit`函数在两秒的过渡期内淡出任何多余的行，将它们的不透明度降低到零，然后移除它们。

![4](img/#co_visualizing_individual_winners_CO2-4)

首先，我们使用获奖者的数据创建一个包含年份、类别和姓名的数据数组，这些数据将用于创建行的`<td>`数据单元格…​

![5](img/#co_visualizing_individual_winners_CO2-5)

…​然后我们将这个数组与行的数据单元格（`td`）连接起来，并用它来设置它们的文本。

![6](img/#co_visualizing_individual_winners_CO2-6)

每次数据更改时，我们从新数据集中随机选择一个获奖者，并在生物框中显示他或她。

当用户将光标移动到我们的获奖者表格中的一行上时，我们希望突出显示该行，并且还要更改指针的样式为`cursor`，以指示该行可以点击。以下 CSS 代码解决了这些细节问题，并添加到了我们的*style.css*文件中：
...
生物框使用获奖者对象填充一个小型传记的细节。生物框的 HTML 骨架提供在*index.xhtml*文件中，包括用于生物元素的内容块和一个`readmore`页脚，提供了一个指向获奖者更多信息的维基百科链接：
...
在*style.css*中的一点点 CSS 设置了列表和生物框元素的位置，调整了它们的内容块大小，并提供了边框和字体的具体设置：
...
确定我们的内容块就位后，我们需要回调我们的数据 API 以获取填充它们所需的数据。示例 20-2 展示了用于构建生物框的`displayWinner`方法。
...
let displayWinner = function (wData) {
  ...
  nw.style('border-color', nbviz.categoryFill(wData.category)) ![1](img/1.png)

  nw.selectAll('.property span').text(function (d) { ![2](img/2.png)
  ...
  if (wData.bio_image) { ![3](img/3.png)
  ...
  nw.select('#readmore a').attr( ![4](img/4.png)
    'href',
...
![1](img/#co_visualizing_individual_winners_CO3-1)

我们的`nobel-winner`元素有一个顶部边框（CSS: `border-top: 4px solid`），我们将根据获奖者的类别使用*nbviz_core.js*中定义的`categoryFill`方法来着色。

![2](img/#co_visualizing_individual_winners_CO3-2)

我们选择所有具有类名`property`的`div`的`<span>`标签。这些标签的形式是`<span name=*category*></span>`。我们使用 span 的`name`属性从我们的诺贝尔获奖者数据中检索正确的属性，并将其用于设置标签的文本。

![3](img/#co_visualizing_individual_winners_CO3-3)

在这里，如果有获奖者的图片可用，我们会设置其`src`（源）属性。如果没有可用的图片，我们使用图像标签的`display`属性将其隐藏（设置为`none`），或者显示它（默认为`inline`）。

![4](img/#co_visualizing_individual_winners_CO3-4)
...
![1](img/#co_visualizing_individual_winners_CO4-1)
