...
查看我们大数组的执行时间，我们看到它完成的速度比Python循环快了几个数量级：
...
逆指数函数，对数函数也是可用的。基本的`np.log`给出自然对数；如果你偏好计算以2为底或以10为底的对数，这些也是可用的：
...
NumPy和`scipy.special`中还有许多其他的ufunc可用。由于这些包的文档可以在线获取，因此通过类似“gamma function python”的网络搜索通常可以找到相关信息。
