...
在Scikit-Learn中，这样一个优化的随机决策树集成是通过`RandomForestClassifier`估计器实现的，它自动处理所有随机化。你只需选择一些估计器，它将非常快速地（如果需要的话是并行的）拟合树的集成（见[图 44-7](#fig_0508-random-forests_files_in_output_30_0)）。
